...
**نام و نام خانوادگی:** سرو ربانی

**شماره دانشجویی:** 40217023134

**نقش:** انجام تمامی مراحل پروژه، تحلیل ترافیک، مستندسازی و پیاده‌سازی بخش برنامه‌نویسی
...
این مقدار نشان می‌دهد حدود 156 میلی‌ثانیه طول کشیده تا درخواست به سرور برسد و اولین پاسخ دریافت شود.
...
---
## 8. تحلیل سناریوی کندی 

اگر زمان رفت و برگشت (Time Delta) بیش از ۲ ثانیه طول بکشد، ریشه‌ی اصلی این کندی می‌تواند به سه بخش کلاینت، مسیر شبکه یا سرور مربوط باشد:

* **سمت کلاینت (سیستم خودمان):** شلوغی پردازنده (CPU) یا پر شدن بافر کارت شبکه باعث معطل شدن و تأخیر در ارسال و دریافت پکت‌ها می‌شود.
* **مسیر و زیرساخت شبکه (پهنای باند):** شلوغی روترهای بین راهی و پهنای باند ضعیف موجب گم شدن بسته‌ها (Packet Loss) می‌شود؛ در نتیجه پروتکل TCP مجبور به ارسال مجدد بسته‌ها (Retransmission) شده و زمان RTT را به شدت بالا می‌برد.
* **سمت سرور (سایت مقصد):** بالا بودن بار ترافیکی روی سخت‌افزار سرور یا طولانی شدن پردازش‌های درونی (مانند اجرای کوئری‌های سنگین دیتابیس و پردازش کدهای بک‌آند)، باعث می‌شود سرور پاسخ نهایی را دیرتر آماده و ارسال کند.
